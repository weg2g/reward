...
const provider = new Provider(process.env.privateKey, process.env.NODEURL);
...
apiRoutes.get("/getCompamies", async function (req, res) {
  ...
  var ss = await Promise.all(
    indexs.map(async (_in) => {
...
apiRoutes.post("/redeem", async function (req, res) {
  accounts = accounts || (await web3.eth.getAccounts());
  let receipt = await getInstance()
    .methods.redeemAd(req.body.aId, req.body.uId)
    .send({ from: process.env.accountAddress, gas: 3000000 });
  console.log(receipt);
  res.status(200).send({
    success: true,
  ...
  if (existingCompany) {
    ...
    _company.save(async function (err, cmp) {
      if (err) {
        ...
        accounts = accounts || (await web3.eth.getAccounts());
        let receipt = await getInstance()
          .methods.addCompany(cmp.id, cmp.name)
          .send({ from: process.env.accountAddress, gas: 3000000 });
        console.log(receipt);
...
apiRoutes.post("/addAd", async function (req, res) {
  ...
  _ad.save(async function (err, ad) {
    if (err) {
      ...
      accounts = accounts || (await web3.eth.getAccounts());
      let receipt = await getInstance()
        .methods.createAd(parseInt(ad.price), ad.text, parseInt(ad.cId))
        .send({ from: process.env.accountAddress, gas: 3000000 });
      console.log(receipt);
      res.json({
        success: true,
...
function sendMail(usr) {
  let mailOptions = {
    ...
    html:
      '<p>You are enrolled with us!! Please use your email address as your user name and password!!<br/><br/>Click <a href="' +
      "//localhost:4200" +
      '">here</a> check out cool offers and redeem them.</p>', // html body
  };
...
async function saveUser(user, cId, accounts) {
  return new Promise(async (resolve) => {
    ...
    if (existingUser) {
      await getInstance()
        .methods.addUser(parseInt(user.uId), user.name, cId, parseInt(user.bal))
        .send({ from: process.env.accountAddress, gas: 3000000 });
      resolve(true);
    } else {
      var _user = new User({
